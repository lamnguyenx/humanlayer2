...
- H1 and H2 headings in all markmap files must aggressively use `<br/>` to break text into multiple short lines, saving horizontal space in the markmap visualization.

### Post-processing
After writing all markmap files, run the project's built-in script to aggressively insert `<br/>` tags into H1/H2 headings:
```bash
python3 local/fix_br_tags.py lectures-markmaps/
```
If that fails, try the absolute path:
```bash
python3 /Users/lamnt45/git/coursera-google-pm/local/fix_br_tags.py lectures-markmaps/
```
